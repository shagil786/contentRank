...
export function TrendingMomentum() {
  ...
  const rising = (data?.rising?.length ? data.rising : live.slice(0, 8))
    .map((r) => entities.find((e) => e.id === r.id) ?? r)
    .filter((r) => r.momentum > 0)
    .slice(0, 8);
  const falling = (data?.falling?.length ? data.falling : [...live].reverse().slice(0, 8))
    .map((r) => entities.find((e) => e.id === r.id) ?? r)
    .filter((r) => r.momentum < 0)
    .slice(0, 8);
          ...
          {rising.length === 0 ? (
            ...
            rising.map((r, i) => (
              ...
              >
                ...
                <Poster entity={r} variant="thumb" className="h-10 w-8 shrink-0" />
