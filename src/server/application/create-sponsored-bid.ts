...
export async function createSponsoredBid(
  ...
  try {
    checkout = await adapters.payments.dodo.createCheckout({
      ...
      bidId: bid.id,
      idempotencyKey,
      successUrl: input.successUrl || `${(process.env.APP_URL || "http://localhost:3000").replace(/\/$/, "")}/?bid=success`,
      cancelUrl: input.cancelUrl || `${(process.env.APP_URL || "http://localhost:3000").replace(/\/$/, "")}/?bid=cancel`,
